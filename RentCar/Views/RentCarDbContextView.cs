...
namespace RentCar.Views.RentCarDbContextView{
    public partial class RentCarDbContextView : XtraUserControl {
        ...
        void InitializeNavigation() {
            ...
            var fluentAPI = mvvmContext.OfType<RentCar.ViewModels.RentCarDbContextViewModel>();
						tileBar.SelectedItem = tileBarItemClientCollectionView;

			            fluentAPI.BindCommand(tileBarItemClientCollectionView, (x, m) => x.Show(m), x => x.Modules[0]);
			            fluentAPI.BindCommand(tileBarItemEmployeeCollectionView, (x, m) => x.Show(m), x => x.Modules[1]);
			            fluentAPI.BindCommand(tileBarItemFuelTypeCollectionView, (x, m) => x.Show(m), x => x.Modules[2]);
			            fluentAPI.BindCommand(tileBarItemVehicleCollectionView, (x, m) => x.Show(m), x => x.Modules[3]);
			            fluentAPI.BindCommand(tileBarItemVehicleBrandCollectionView, (x, m) => x.Show(m), x => x.Modules[4]);
			            fluentAPI.BindCommand(tileBarItemVehicleModelCollectionView, (x, m) => x.Show(m), x => x.Modules[5]);
			            fluentAPI.BindCommand(tileBarItemVehicleTypeCollectionView, (x, m) => x.Show(m), x => x.Modules[6]);
			            fluentAPI.BindCommand(tileBarItemInspectionCollectionView, (x, m) => x.Show(m), x => x.Modules[7]);
			            fluentAPI.BindCommand(tileBarItemRentCollectionView, (x, m) => x.Show(m), x => x.Modules[8]);
