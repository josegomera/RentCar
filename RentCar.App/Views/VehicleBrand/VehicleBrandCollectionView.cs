...
namespace RentCar.App.Views.VehicleBrandCollectionView{
    public partial class VehicleBrandCollectionView : XtraUserControl {
        public VehicleBrandCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<RentCar.App.ViewModels.VehicleBrandCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as RentCar.DataModel.Entities.VehicleBrand,
