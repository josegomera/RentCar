...
namespace RentCar.App.Views.VehicleTypeCollectionView{
    public partial class VehicleTypeCollectionView : XtraUserControl {
        public VehicleTypeCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<RentCar.App.ViewModels.VehicleTypeCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as RentCar.DataModel.Entities.VehicleType,
