...
namespace RentCar.App.Views.FuelTypeCollectionView{
    public partial class FuelTypeCollectionView : XtraUserControl {
        public FuelTypeCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<RentCar.App.ViewModels.FuelTypeCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as RentCar.DataModel.Entities.FuelType,
