...
namespace RentCar.App.Views.InspectionCollectionView{
    public partial class InspectionCollectionView : XtraUserControl {
        public InspectionCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<RentCar.App.ViewModels.InspectionCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as RentCar.DataModel.Entities.Inspection,
