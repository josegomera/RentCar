...
namespace RentCar.App.Views.RentCollectionView{
    public partial class RentCollectionView : XtraUserControl {
        public RentCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<RentCar.App.ViewModels.RentCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as RentCar.DataModel.Entities.Rent,
